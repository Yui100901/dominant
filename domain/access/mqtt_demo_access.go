...
type MQTTDemoClient struct {
	ID         string
	MqttClient *mqtt_utils.MQTTClient
}

func NewMQTTDemoClient(id string) *MQTTDemoClient {
	if id != "" {
		config.Config.MQTT.Node.ID = id
	}
	return &MQTTDemoClient{
		ID: id,
...
func (n *MQTTDemoClient) Receive() {
	n.MqttClient.SubscribeDefault()
}

func (n *MQTTDemoClient) MQTTSend(r *mqtt_utils.MQTTPublishRequest) ([]byte, error) {
